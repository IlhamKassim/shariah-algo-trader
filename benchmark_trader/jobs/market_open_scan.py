...
from benchmark_trader.signals.gap_and_go import compute_gap, is_valid_gap_entry
from benchmark_trader.state import ActivePosition, BenchmarkState
...
def run_market_open_scan(
    ...
    Gap and Go enters at the open on stocks with a significant overnight gap
    and confirmed first-minute participation (RVOL). No waiting for a range.
    """
    if state.is_stale():
        state.reset()
    ...
    # Fetch first-minute bar (9:30–9:31) for volume, previous closes, and current prices
